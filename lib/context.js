...
Context.prototype.send = function(msg, cb) {
  if (this.pending) {
    this.sendQueue.push({msg: msg, cb: cb});
  } else {
    this.pending = cb;
...
Context.prototype.verbose = function(msg, level, cb) {
  ...
  if (isNaN(level))
    level = 1;
  this.send('VERBOSE ' + parseArgument(msg) + ' ' + level + '\n', typeof cb == "function" ? cb : function() {});
}

Context.prototype.getVariable = function(name, cb) {
  this.send('GET VARIABLE ' + parseArgument(name) + '\n', cb || function() { });
};

Context.prototype.getFullVariable = function(expr, cb) {
  this.send('GET FULL VARIABLE ' + parseArgument(expr) + '\n', cb || function() { });
}

Context.prototype.setVariable = function(name, value, cb) {
  this.send('SET VARIABLE ' + parseArgument(name) + ' ' + parseArgument(value) + '\n', cb || function() { })
}
